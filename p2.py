...
def forwadChecking(row,col,num,dt,table,ctsble,domainTable):
    for i in range(n):
        while True:
            try:
...
#prototype
def sazegar_konande_yek_cell(row,col,colortable,colordomainTable,numtable,m):
...
def co(table,domainTable,colortable,colordomainTable,m):
    ...
    for assinNum in colordomainTable[row][col]:
        ...
        if (updated!=0):
            if co(table,updated,colortable,colordomainTable,m)!= 0:
                return updated
    colortable[row][col]=0
    return 0


def f(table,domainTable,colortable,colordomainTable,m):
    row,col = MRV(domainTable,table,n)
    if row == n+1:
        # nums filled
        if co(table,domainTable,colortable,colordomainTable,m)!= 0:
                return 1
        return 0
    
    for assinNum in domainTable[row][col]:
        table[row][col] = assinNum
        updated = forwadChecking(row,col,assinNum,domainTable,table,colortable,colordomainTable)
        if (updated!=0):
            if f(table,updated,colortable,colordomainTable,m)!= 0:
                return 1
    table[row][col]=0
    return 0
...
if __name__=="__main__":
    ...
    f(numtable,numdomainTable,colortable,colordomainTable,m)
    print(numtable)
    print(colortable)